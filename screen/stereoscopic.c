...
void draw_buffer_right(char *addr, int w, int h, int pitch)
{
	cairo_t *cr;
...
void draw_buffer_left(char *addr, int w, int h, int pitch)
{
	int ret, i, j;
	pixel_t d = {255,255,255,255};
...
int main(int argc, char *argv[])
{
	...
	fprintf(stderr, "drmOpen i915\n");
#else
	fd = open("/dev/dri/card0", O_RDWR);
	fprintf(stderr, "drmOpen /dev/dri/card1\n");
#endif
	if(fd < 0){
	...
	create_bo(kms_driver, mode.hdisplay, mode.vdisplay, 
		&pitch, &kms_bo, &bo_handle, draw_buffer_right);
	...
	create_bo(kms_driver, mode.hdisplay, mode.vdisplay, 
		&pitch, &second_kms_bo, &bo_handle, draw_buffer_left);
